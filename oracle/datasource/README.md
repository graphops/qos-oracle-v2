...
- [`Datasource`](./src/datasource.rs): exposes methods for retrieving a list of `RequestTicket`, `RequestTicketStat` and `RequestTicketSubgraphStat` records pulled and parsed from the data storage model determined by the implementer of the `Datasource`.
  - methods:
    - `user`: retrieves a derived User record from the `ticket_user` ran queries
    - `query_keys`: retrieves a list of unique `QueryKey` records
...
Instantiate a new [`GraphServiceDatasource`](./src/lib.rs) using whichever datasource implementer suits your needs for the `graph-subscriptions-api`:

```rust
use datasource:{GraphServiceDatasource, RequestTicket};
...
async fn main() -> anyhow::Result<()> {
  ...
  let services_datasource = GraphServiceDatasource::create_with_datasource_pg(
    "localhost:9092".parse::<Url>().unwrap(), // kafka logs consumer url
    String::from("graph_services_log_group"), // kafka group id
    String::from("gateway_client_query_results"), // kafka topic id
    "postgress://dev:dev@0.0.0.0:5432/gateway_client_query_results", // database url
    Some(2) // number of async workers to listen to messaged and write to the postgres instance
  ).await?;
  // get the request tickets
  let user_address = "0xa476caFd8b08F11179BDDd5145FcF3EF470C7462".parse::<Address>()?;
  let query_keys: Vec<QueryKey> = services_datasource.datasource.query_keys(user, None, None, None, None)
    .await
    .unwrap_or_default();
}
```
